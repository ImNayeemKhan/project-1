...
export default function AdminTicketDetailPage() {
  ...
  const key = `/api/admin/tickets/${id}`;
  ...
  const updateStatus = async (status: Ticket['status']) => {
    try {
      await api.patch(`/api/admin/tickets/${id}`, { status });
      mutate(key);
    } catch (e) {
      setErr(extractError(e));
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-1 text-sm text-slate-500">
          <span className="font-mono">{t.ticketNo}</span> · {t.customer?.name} ({t.customer?.email}) · {t.category}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {(['open', 'pending', 'resolved', 'closed'] as const).map((s) => (
          <button
            ...
            className={`btn text-xs ${t.status === s ? 'btn-primary' : 'btn-secondary'}`}
          >
            {s}
          </button>
        ))}
